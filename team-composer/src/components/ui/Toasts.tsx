...
const TOAST_CLASSES: Record<ToastType, string> = {
  success: 'border-emerald-500 bg-emerald-900',
  info: 'border-blue-500 bg-sky-950',
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[1000] flex flex-col gap-2">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={cn('max-w-[400px] rounded-lg border px-4 py-2.5 text-sm font-medium text-slate-200', TOAST_CLASSES[toast.type])}
        >
          {toast.message}
        </div>
      ))}
